...
const Toggle = ({ value, onChange }: { value: boolean, onChange: (val: boolean) => void }) => (
  <div
    ...
    style={{
      ...
      height: '24px',
      borderRadius: '12px',
      background: value ? '#7c3aed' : '#21262d',
      position: 'relative',
      cursor: 'pointer',
...
const SettingRow = ({ title, description, control, isLast = false }: any) => (
  <div style={{
    ...
    <div>
      <div style={{ color: '#e6edf3', fontSize: '13px', fontWeight: 600, margin: '0 0 2px 0' }}>
        {title}
      </div>
      <div style={{ color: '#7d8590', fontSize: '12px', margin: 0 }}>
        {description}
      </div>
    </div>
...
const SectionCard = ({ icon, title, subtitle, children }: any) => (
  <div style={{
    ...
    <div style={{ fontSize: '12px', color: '#7d8590', marginBottom: '16px', marginLeft: '26px' }}>{subtitle}</div>
    <div>{children}</div>
  </div>
);
...
export default function Settings() {
  ...
  return (
    <div style={{ background: '#0d1117', minHeight: '100%', width: '100%' }}>
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '28px 24px' }}>
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ fontSize: '13px', color: '#7d8590' }}>Manage your preferences</div>
        </div>

        {/* 1. Notifications */}
        <SectionCard 
          icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#20c997" strokeWidth="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg>}
          title="Notifications"
        ...
        {/* 2. Security */}
        <SectionCard 
          icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#20c997" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>}
          title="Security"
          subtitle="Protect your account"
        >
          <SettingRow title="Two-factor authentication" description="Add an extra layer of security" control={
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              {tfa && <span style={{ fontSize: '11px', background: '#21262d', color: '#e6edf3', padding: '2px 8px', borderRadius: '12px' }}>Coming soon</span>}
              <Toggle value={tfa} onChange={setTfa} />
            </div>
          } />
          
          <div style={{ paddingTop: '14px' }}>
            <div style={{ color: '#e6edf3', fontSize: '13px', fontWeight: 600, marginBottom: '12px' }}>Change Password</div>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
              <input 
                ...
                style={{ background: '#0d1117', border: '1px solid #21262d', borderRadius: '6px', color: '#e6edf3', padding: '8px 12px', fontSize: '13px', flex: 1, minWidth: '150px' }} 
              />
              <input 
                type="password" 
              ...
              <button 
                ...
                style={{ background: '#21262d', color: '#e6edf3', border: '1px solid #30363d', borderRadius: '6px', padding: '8px 16px', fontSize: '13px', cursor: 'pointer', whiteSpace: 'nowrap' }}
              >
                Update
              </button>
            </div>
            {pwStatus.message && (
              <div style={{ marginTop: '8px', fontSize: '12px', color: pwStatus.type === 'success' ? '#20c997' : '#f85149' }}>
                {pwStatus.message}
              </div>
            )}
          </div>
        ...
        {/* 3. Appearance */}
        <SectionCard 
          icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#20c997" strokeWidth="2"><path d="M12 2.69l5.66 5.66a8 8 0 1 1-11.31 0z"></path></svg>}
          title="Appearance"
          subtitle="Customize your experience"
        >
          <div style={{ display: 'flex', gap: '16px' }}>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', color: '#e6edf3', fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>Language</label>
              <select 
                value={appearance.language} 
                onChange={e => setAppearance({...appearance, language: e.target.value})}
                style={{ width: '100%', background: '#0d1117', border: '1px solid #21262d', borderRadius: '6px', color: '#e6edf3', padding: '8px 12px', fontSize: '13px' }}
              >
                <option>English</option>
              </select>
            </div>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', color: '#e6edf3', fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>Timezone</label>
              <select 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginTop: '32px' }}>
          ...
          <div style={{ color: '#20c997', fontSize: '13px', opacity: saveToastVisible ? 1 : 0, transition: 'opacity 0.2s', fontWeight: 500 }}>
             Settings saved
          </div>
        </div>
      </div>

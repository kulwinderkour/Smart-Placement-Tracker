...
// ─── Theme ───────────────────────────────────────────────────────────────────
// Aligned with the unified student palette (--student-* tokens in index.css)
const C = {
  bg: 'var(--student-bg)', card: 'var(--student-surface)', border: 'var(--student-border)', borderH: 'var(--student-border-strong)',
  text: 'var(--student-text)', textSecondary: 'var(--student-text-secondary)', muted: 'var(--student-text-muted)', dim: 'var(--student-text-dim)',
  primary: '#a78bfa', primaryHover: '#7c3aed',
  accentBg: 'rgba(167, 139, 250, 0.10)',
  warn: '#f0b429', danger: '#f85149', blue: '#58a6ff', success: '#3fb950',
}
...
const CATS: CatDef[] = [
  ...
  { id: 'resume', label: 'Resume',         icon: '📄', color: '#a78bfa' },
...
const MOODS: { id: Mood; label: string; icon: string; color: string }[] = [
  { id: 'locked',     label: 'Locked In',  icon: '🔥', color: '#a78bfa' },
  { id: 'average',    label: 'Average',    icon: '😐', color: '#58a6ff' },
  { id: 'distracted', label: 'Distracted', icon: '😵', color: '#f0b429' },
]
...
function readinessMsg(s: number): { msg: string; color: string } {
  if (s >= 80) return { msg: 'Interview-ready this week 🚀', color: '#3fb950' }
  if (s >= 60) return { msg: "You're building strong momentum ⚡", color: '#a78bfa' }
  if (s >= 40) return { msg: "Keep pushing — you're getting there 💪", color: '#f0b429' }
  return { msg: 'Your prep needs attention. Start today!', color: '#f85149' }
}
...
function MomentumRing({ score }: { score: number }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      ...
      <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: C.textSecondary, textAlign: 'center', maxWidth: 190 }}>{msg}</p>
    </div>
  )
}
...
function ZoneRow({ title, emoji, cat, color }: ZProps) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', background: 'var(--student-surface-hover)', borderRadius: 9, border: `1px solid ${C.border}` }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: C.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cat.icon} {cat.label}</p>
      </div>
      <span style={{ fontSize: 14, fontWeight: 800, color, background: `${color}22`, padding: '2px 8px', borderRadius: 20 }}>{cat.score}</span>
    </div>
  )
}
    ...
    <div style={{ background: C.bg, minHeight: '100vh', color: C.text }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px 60px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[
              ...
              { label: 'Days This Week',  val: `${activeDays}/7`, icon: '📅', col: C.success },
            ].map(s => (
              <div key={s.label} style={{
                ...
                <div>
                  <p style={{ margin: 0, fontSize: 10, color: C.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{s.label}</p>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: C.textSecondary, whiteSpace: 'nowrap' }}>7-Day Activity</span>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
            <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: C.text }}>Today's Prep Actions</h2>
            ...
            {ACTIONS.map(a => {
              ...
              return (
                <button key={a.id} onClick={() => doAction(a.id, a.cats, a.pts)}
                  style={{
                    background: done ? C.accentBg : C.bg,
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, padding: '20px', marginBottom: 20 }}>
          <h2 style={{ margin: '0 0 18px', fontSize: 16, fontWeight: 700, color: C.text }}>Category Readiness</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            {CATS.map(cat => {
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, padding: '20px' }}>
          <h2 style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 700, color: C.text }}>Today's Reflection</h2>
            ...
            {MOODS.map(m => (
              <button key={m.id}
                ...
                style={{
                  ...
                  color: st.mood === m.id ? m.color : C.textSecondary,
